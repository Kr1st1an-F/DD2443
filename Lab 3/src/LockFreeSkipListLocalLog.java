...
public class LockFreeSkipListLocalLog<T extends Comparable<T>> implements LockFreeSet<T> {
    ...
    public boolean remove(int threadId, T x) {
        ...
        while (true) {
            ...
            if (!found) {
                ...
                while (true) {
                    // Log around the unsuccessful linearization point of remove
    ...
    public Log.Entry[] getLog() {
        List<Log.Entry> globalLog = new ArrayList<>(); // The global log
        for (List<Log.Entry> localLog : threadLogs.values()) { // Merge all logs
    ...
    public void reset() {
        for (int i = 0; i < head.next.length; i++) { // Reset the head node
            head.next[i] = new AtomicMarkableReference<LockFreeSkipListLocalLog.Node<T>>(tail, false);
        }
        threadLogs.clear(); // Clear the logs
    }

    private void logAction(Log.Method method, int key, boolean result, long timestamp) {
        if (timestamp <= 0.0) timestamp = System.nanoTime(); // Use the current time if not provided
        long threadId = Thread.currentThread().getId();
        threadLogs.putIfAbsent(threadId, new ArrayList<>()); // Create a new log if not exists
        List<Log.Entry> localLog = threadLogs.get(threadId); // Get the log
